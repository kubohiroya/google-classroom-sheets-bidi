...
export function exportFormWithDialog(): void {
  ...
  try {
    ...
    openUrl("Please wait to open the forms page...", form.getPublishedUrl());
  } catch (exception) {
...
export function updateForm(): void {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const { createdAt, updatedAt } = getCreatedAtUpdatedAtValues(ss.getId());
    const sheet = ss.getActiveSheet();
...
export function previewForm(): void {
